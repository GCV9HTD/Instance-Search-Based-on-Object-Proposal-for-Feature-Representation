...
class COCOvalDataset(Dataset):
    def __init__(self, list_path, img_size, is_debug=False):
        ...
        for path in open(list_path, 'r'):
            ...
            # voc
            # label_path = path.replace('JPEGImages', 'Labels').replace('.png', '.txt').replace('.jpg', '.txt').strip()
...
if __name__ == "__main__":
    dataloader = torch.utils.data.DataLoader(COCOvalDataset("../data/coco/trainvalno5k.txt", (416, 416), is_debug=True),
