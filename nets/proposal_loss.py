    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        for b in range(bs):
            for t in range(target.shape[1]):
                ...
                # gj before gi
                # print b, anchors > ignore_threshold, gj, gi
                try:
                    noobj_mask[b, anch_ious > ignore_threshold, gj, gi] = 0
                except:
                    print 'noobj cord', in_h, in_w, gj, gi
                    continue
                ...
                try:
                    ...
                    print 'obj cord', in_h, in_w, gj, gi
                    continue
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        if targets is not None:
            #  build target
            obj_mask, noobj_mask, tx, ty, tw, th, tconf, tcls = self.get_target(targets, scaled_anchors, in_w, in_h, self.ignore_threshold)
            obj_mask, noobj_mask = obj_mask.cuda(), noobj_mask.cuda()
            tx, ty, tw, th = tx.cuda(), ty.cuda(), tw.cuda(), th.cuda()
            ...
            loss_conf = self.lambda_obj * self.bce_loss(conf * obj_mask, obj_mask * 1.0) + \
